...
class Evaluation:
    ...
    def start(self):
        ...
        for testPro in testingProjectNames:
            ...
            self.record_acc("evaluation", testPro, precision, recall)

            # for evaluation of baseline
            # baseline_pre_lst = self.getBaseline_lst(self.custom_args['BaseLine_PATH'], testPro, self.n)
            # if len(GT_set) < self.n or len(baseline_pre_lst) < self.n:
            #     continue
            # intersections = GT_set.intersection(set(baseline_pre_lst))
            # precision = 1.0 * len(intersections) / self.n
            # recall = 1.0 * len(intersections) / len(GT_set)
            # self.record_acc("baseline_evaluation", testPro, precision, recall)
